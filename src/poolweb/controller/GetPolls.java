...
@WebServlet(name = "GetPolls")
public class GetPolls extends PoolWebBaseController{
    @Override
    ...
    private void action_poll(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        try {
            ...
            if (poll != null) {
                ...
                request.setAttribute("page_title", "Tutti i sondaggi"); //Titolo da iniettare nel template con freeMarker
                TemplateResult res = new TemplateResult(getServletContext());
                request.setAttribute("strip_slashes", new SplitSlashesFmkExt());
                res.activate("show_polls.ftl", request, response);
            } else {
